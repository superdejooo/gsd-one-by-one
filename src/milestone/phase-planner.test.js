...
const { parsePhaseNumber, executePhaseWorkflow } = await import('./phase-planner.js');
...
describe('phase-planner.js', () => {
  ...
  describe('executePhaseWorkflow', () => {
    ...
    beforeEach(() => {
      ...
      mockReadFile.mockResolvedValue('Phase planning completed successfully\n\nNext steps:\n- Review the plan');
    ...
    it('logs workflow progress', async () => {
      ...
      expect(vi.mocked(core.info)).toHaveBeenCalledWith('Starting phase planning workflow for test-owner/test-repo#42');
      expect(vi.mocked(core.info)).toHaveBeenCalledWith('Parsed phase number: 9');
      expect(vi.mocked(core.info)).toHaveBeenCalledWith('Phase 9 planning workflow complete');
    });
