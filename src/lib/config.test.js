...
describe('loadConfig', () => {
  ...
  it('loads config from .github/gsd-config.json when present', async () => {
    ...
    const encodedContent = Buffer.from(JSON.stringify(customConfig)).toString('base64');
    ...
    expect(core.info).toHaveBeenCalledWith('Loaded config from .github/gsd-config.json');
  });
  ...
  it('throws error for non-404 failures', async () => {
    ...
    await expect(loadConfig('owner', 'repo')).rejects.toThrow('Failed to load config');
  });
});
