...
const { STATUS_LABELS, ensureLabelsExist, applyLabels, updateIssueStatus } =
...
describe("ensureLabelsExist", () => {
  ...
  it("skips existing labels", async () => {
    ...
    mockOctokit.rest.issues.listLabelsForRepo.mockResolvedValue({
      data: [{ name: "status:pending", color: "d4c5f9" }],
    });
    ...
    expect(core.info).toHaveBeenCalledWith(
      "Label already exists: status:pending",
    );
  });
...
describe("applyLabels", () => {
  ...
  it("calls addLabels with correct params", async () => {
    ...
    await applyLabels("test-owner", "test-repo", 123, [
      "status:pending",
      "enhancement",
    ]);
  ...
  it("logs applied labels", async () => {
    ...
    expect(core.info).toHaveBeenCalledWith(
      "Applied labels to issue #123: status:pending, enhancement",
    );
  });
});
...
describe("updateIssueStatus", () => {
  ...
  it("logs status update", async () => {
    ...
    expect(core.info).toHaveBeenCalledWith(
      "Updated issue #123 status to: status:in-progress",
    );
  });
});
