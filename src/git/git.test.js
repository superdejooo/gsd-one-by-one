...
const { runGitCommand, configureGitIdentity, createAndSwitchBranch, switchBranch } = await import('./git.js');
...
describe('git.js', () => {
  ...
  describe('runGitCommand', () => {
    it('returns stdout trimmed on success', async () => {
      ...
      expect(vi.mocked(core.debug)).toHaveBeenCalledWith('Executing: git branch');
    });
    ...
    it('logs warning when stderr has content', async () => {
      ...
      expect(vi.mocked(core.warning)).toHaveBeenCalledWith('Git command warning: warning message');
    });
    ...
    it('throws error when command fails', async () => {
      ...
      expect(vi.mocked(core.error)).toHaveBeenCalledWith('Git command failed: git invalid');
  ...
  describe('configureGitIdentity', () => {
    it('runs git config for user.name', async () => {
      ...
      expect(mockExec).toHaveBeenCalledWith('git config set user.name "Test User"');
    });
    ...
    it('runs git config for user.email', async () => {
      ...
      expect(mockExec).toHaveBeenCalledWith('git config set user.email "test@example.com"');
    });
    ...
    it('logs success message', async () => {
      ...
      expect(vi.mocked(core.info)).toHaveBeenCalledWith('Git identity configured: Test User <test@example.com>');
    });
  });
  ...
  describe('createAndSwitchBranch', () => {
    ...
    it('includes startPoint when provided', async () => {
      ...
      expect(mockExec).toHaveBeenCalledWith('git switch -c feature-branch main');
    });
    ...
    it('logs branch creation', async () => {
      ...
      expect(vi.mocked(core.info)).toHaveBeenCalledWith('Created and switched to branch: feature-branch');
    });
  });
  ...
  describe('switchBranch', () => {
    ...
    it('logs branch switch', async () => {
      ...
      expect(vi.mocked(core.info)).toHaveBeenCalledWith('Switched to branch: main');
    });
  });
});
