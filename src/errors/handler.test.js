...
describe('withErrorHandling', () => {
  ...
  it('posts formatted error comment for technical errors', async () => {
    ...
    expect(formatter.formatErrorComment).toHaveBeenCalledWith(error, 'https://example.com/run/123');
  ...
  it('posts userMessage for AuthorizationError (not formatted)', async () => {
    const authError = new AuthorizationError('alice', 'read', 'User does not have write access');
  ...
  it('logs info when authorization error detected', async () => {
    ...
    expect(core.info).toHaveBeenCalledWith('Authorization error detected - posting user-friendly message');
  });
});
