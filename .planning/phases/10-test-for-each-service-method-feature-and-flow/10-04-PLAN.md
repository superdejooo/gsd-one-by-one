...
<task type="auto">
  <name>Task 0: Fix branchExists import bug</name>
  <files>src/git/branches.js</files>
  <action>
  Fix missing import in src/git/branches.js. The branchExists function uses runGitCommand() but it's not imported.
...
This bug would cause a ReferenceError at runtime when branchExists is called.
</action>
<verify>Import src/git/branches.js without ReferenceError</verify>
<done>Source bug fixed, branchExists can now call runGitCommand</done>
</task>
...
// NOTE: @actions/core is already mocked globally in test/setup.js
// No need to mock it again here - just import and use vi.mocked() if needed
...
- Source bug in branches.js fixed (runGitCommand import)
