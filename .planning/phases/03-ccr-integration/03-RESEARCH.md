...
export async function executeTask(prompt, options = {}) {
  ...
  const result = query({
    ...
    options: {
      ...
      allowedTools: options.allowedTools || ["Read", "Write", "Bash", "Glob", "Grep"],
...
export async function executeLLMTaskWithRetry(prompt, options = {}, maxRetries = 3) {
  let lastError;
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (result.output.includes("429") || result.output.includes("rate limit")) {
        throw new Error("Rate limited");
      }
      if (result.output.includes("529") || result.output.includes("overloaded")) {
